# HyperFrisette – un réseau férré alternatif
...
L'utilisateur (compagnie férroviaire), peut demander d'ajouter des options supplémentaires pour la fonctionnalité de déplacement : connexion au réseau Wi-Fi d'HyperFrisette, connexion au service de streaming vidéo d'HyperFrisette, etc. Ajoutez la méthode `deplacer()` dans la classe `Train` et pensez à la rendre suffisamment flexible pour prendre en compte toutes ces options. Pour mettre en  ́evidence ces options, vous pourrez vous contenter d’un simple message d’affichage correspondant.
